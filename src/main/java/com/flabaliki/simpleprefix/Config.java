...
public class Config {
    ...
    private void loadKeys() {
        ...
        SimplePrefix.multiPrefix = config.getBoolean("Template.multiPrefix");
        SimplePrefix.multiPrefixSeparator = config.getString("Template.multiPrefixSeparator");
        SimplePrefix.multiSuffix = config.getBoolean("Template.multiSuffix");
    ...
    public String getVaultPrefix(Player player) {
        ...
        if (groupPrefixes.contains(userPrefix))
            userPrefix = ""; // b/c sometimes Vault decides to return a group prefix as the user prefix
    ...
    public String getVaultSuffix(Player player) {
        ...
        if (groupSuffixes.contains(userSuffix))
            userSuffix = ""; // b/c sometimes Vault decides to return a group suffix as the user suffix
    ...
    private String getPrefixSuffix(Player player, String type) {
        if (SimplePrefix.debug)
            SimplePrefix.log.info("[" + SimplePrefix.pluginName + "] DEBUG: Attempting to get " + type + " for " + player.getName() + " from SimplePrefix config");

        String prefixSuffix = "";
        Set<String> groups = config.getConfigurationSection("Group").getKeys(false);
        List<String> usedTiers = new LinkedList<>();
        for (String group : groups) {
            if (player.hasPermission("simpleprefix." + group) && (config.getString("Group." + group + "." + type) != null)) {
                if (!SimplePrefix.allowOps && !player.isPermissionSet("simpleprefix." + group))
                    continue;
                if (SimplePrefix.debug)
                    SimplePrefix.log.info("[" + SimplePrefix.pluginName + "] DEBUG: Player has permission for the '" + group + "' prefix/suffix group");
                if ((type.equals("prefix") && SimplePrefix.multiPrefix) || (type.equals("suffix") && SimplePrefix.multiSuffix)) {
                    ...
                    if (tier == null || tier.equals("") || !usedTiers.contains(tier)) {
                        if (prefixSuffix == null || prefixSuffix.isEmpty())
                            ...
                            if (type.equals("prefix"))
                                prefixSuffix = prefixSuffix + SimplePrefix.multiPrefixSeparator + config.getString("Group." + group + "." + type);
                            else
                                prefixSuffix = prefixSuffix + SimplePrefix.multiSuffixSeparator + config.getString("Group." + group + "." + type);
                        }
                        usedTiers.add(tier);
                    }
                } else {
                    prefixSuffix = config.getString("Group." + group + "." + type);
                    if (SimplePrefix.debug)
                        SimplePrefix.log.info("[" + SimplePrefix.pluginName + "] DEBUG: Multi" + type + " is not enabled, so " + type + " retrieved is '" + prefixSuffix + "'");
                    break;
                }
            }
        }
        ...
        if (config.get("User") != null) {
            String data;
            if (plugin.uuids.containsKey(player.getName()) && config.getConfigurationSection("User").contains(plugin.uuids.get(player.getName()).toString())) {
            ...
            if ((data != null) && (!data.equalsIgnoreCase(""))) {
                if ((type.equals("prefix") && SimplePrefix.multiPrefix) || (type.equals("suffix") && SimplePrefix.multiSuffix)) {
                    if (prefixSuffix == null || prefixSuffix.isEmpty())
                        ...
                        if (type.equals("prefix"))
                            player.setMetadata(type, new FixedMetadataValue(plugin, prefixSuffix + SimplePrefix.multiPrefixSeparator + data));
                        else if (type.equals("suffix"))
                            player.setMetadata(type, new FixedMetadataValue(plugin, prefixSuffix + SimplePrefix.multiSuffixSeparator + data));
                    }
                } else {
                    player.setMetadata(type, new FixedMetadataValue(plugin, data));
                    if (SimplePrefix.debug)
                        SimplePrefix.log.info("[" + SimplePrefix.pluginName + "] DEBUG: Multi" + type + " is not enabled, so user " + type + " retrieved is '" + data + "'");
                }
            }
